...
async function run() {
  ...
  const products = await Product.find({ name: /fruit/i }).lean();
  console.log(`Found ${products.length} products matching "fruit":`);
  for (const p of products) {
    ...
    if (p.vendor) {
      ...
      console.log(`Vendor Email: ${vendor ? vendor.email : 'N/A'}`);
