...
import { Send, Mail, User, AlertCircle } from 'lucide-react';
...
const SendMessageModal = ({ show, onHide, customer, type, onSubmit }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            if (type === 'Email') {
                await customerApi.sendEmailToCustomer(adminUser.token, customer._id, {
                    subject,
                    message
                });
                toast.success('Email sent successfully');
            } else {
                await customerApi.sendMessageToCustomer(adminUser.token, customer._id, {
        ...
        <Modal show={show} onHide={onHide} centered className="send-message-modal">
            <Modal.Header closeButton className="border-0 pb-0 px-4">
                <Modal.Title className="fw-black d-flex align-items-center gap-2 uppercase tracking-tight text-[#0c831f]">
                    ...
                    {t('customers.all.message_modal.title', { type: type === 'Email' ? 'Email' : 'Message', name: customer?.name })}
                </Modal.Title>
            </Modal.Header>
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    {type === 'Email' ? (
                        <Form.Group className="mb-3">
                            <Form.Label className="xs font-black text-muted uppercase tracking-widest mb-1">Subject</Form.Label>
                            <Form.Control
                                type="text"
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="xs font-black text-muted uppercase tracking-widest mb-1">Message Title</Form.Label>
                            <Form.Control
                                type="text"
                    ...
                    <Form.Group className="mb-0">
                        <Form.Label className="xs font-black text-muted uppercase tracking-widest mb-1">{type === 'Email' ? 'Email Content' : 'Message Body'}</Form.Label>
                        <Form.Control
                            as="textarea"
                            rows={5}
                            placeholder={`Type your ${type.toLowerCase()} here...`}
                            value={message}
                    ...
                    <div className="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                        <Button variant="light" onClick={onHide} disabled={loading} className="px-4 py-2 border shadow-none fw-black uppercase xs tracking-tighter text-muted">
                            {t('customers.all.message_modal.cancel')}
                        </Button>
                        <Button 
                            variant={type === 'Email' ? 'info' : 'success'} 
                            type="submit" 
                            disabled={loading}
                            className="px-4 py-2 fw-black uppercase tracking-widest d-flex align-items-center gap-2 shadow-sm text-white border-0"
                        >
                            {loading ? <Spinner size="sm" /> : <Send size={18} />} 
                            {t('customers.all.message_modal.send_btn', { type: type })}
                        </Button>
                    </div>
                </Form>
            </Modal.Body>
