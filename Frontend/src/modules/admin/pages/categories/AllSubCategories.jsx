...
import { Search, Plus, Edit, Trash2, ImageIcon, Info, ChevronLeft, ChevronRight, Layers, Package, RefreshCw, Box } from 'lucide-react';
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    {adminUser?.role === 'Admin' && (
                        ...
                        >
                            <Plus size={18} /> {t('subcategories.add_new')}
                        </Link>
                    )}
                </div>
            </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th className="px-4 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-tight">{t('subcategories.table.parent')}</th>
                            ...
                            {loading ? (
                                    ...
                                    <tr key={sc._id} className="hover:bg-slate-50/20 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="w-11 h-11 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center p-2 group-hover:border-blue-200 transition-all shadow-sm">
                                                    {sc.image ? <img src={sc.image} className="max-h-full max-w-full object-contain" alt="" /> : <Package size={18} className="text-slate-300" />}
                                                </div>
                                                <div className="min-w-0">
                                                    <div className="text-sm font-bold text-slate-900 flex items-center gap-1.5 uppercase tracking-tight">
                                                        {sc.name}
                                                        {sc.description && (
                                                            <div className="relative group/scinfo">
                                                                <Info size={14} className="text-slate-200 cursor-help hover:text-blue-500 transition-colors" />
                                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-900 text-white text-[10px] rounded-lg opacity-0 invisible group-hover/scinfo:opacity-100 group-hover/scinfo:visible transition-all z-10 shadow-xl font-medium">
                                                                    {sc.description}
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="text-[10px] font-bold text-slate-300 uppercase tracking-tight">ID: {sc._id.slice(-8)}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[11px] font-bold text-slate-600 uppercase tracking-tight">{sc.categoryName || sc.category?.name || '---'}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                             <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100 uppercase tracking-tight">/{sc.slug}</span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <SubStatusBadge status={sc.status} />
                                        </td>
                                ...
                                <tr>
                                    <td colSpan="5" className="py-24 text-center">
                                        ...
                                        <p className="text-xs font-medium text-slate-400 mt-1">{searchTerm ? 'Try adjusting your search query' : 'Identify primary groupings for your catalog'}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
