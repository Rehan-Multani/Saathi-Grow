...
const AdminNotifications = () => {
    ...
    const handleMarkAsRead = async (id) => {
        try {
            ...
            toast.success(t('admin_inbox.read_success'));
        } catch (error) {
    ...
    const handleMarkAllRead = async () => {
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: t('admin_inbox.all_read_success'),
                timer: 1500,
    ...
    const handleDelete = async (idsToDelete) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                if (res.success) {
                    toast.success(t('admin_inbox.delete_success'));
                    setSelectedIds([]);
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <div className="flex items-center gap-2">
                        <h1 className="text-xl font-bold tracking-tight text-slate-900">{t('admin_inbox.title')}</h1>
                        {totalItems > 0 && (
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    >
                        <CheckCircle size={16} />
                        <span>{t('admin_inbox.mark_all_read')}</span>
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 mb-6 flex items-center justify-between gap-4">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2 pl-3">
                        ...
                        />
                        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-tight">{t('admin_inbox.select_all')}</span>
                    </div>
                    ...
                    {selectedIds.length > 0 && (
                        <div className="flex items-center gap-2 animate-in slide-in-from-left duration-200 border-l border-slate-100 pl-6">
                            ...
                            >
                                <Trash2 size={12} />
                                {t('admin_inbox.delete')} ({selectedIds.length})
                            </button>
                        </div>
                    )}
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100 font-bold text-slate-500 uppercase text-[11px]">
                                ...
                                <th className="px-6 py-4">{t('admin_inbox.subject')}</th>
                                <th className="px-6 py-4 w-48 text-center">{t('admin_inbox.timestamp')}</th>
                                <th className="px-6 py-4 w-28 text-right">{t('admin_inbox.actions')}</th>
                            </tr>
                        </thead>
                            ...
                            {loading && notifications.length === 0 ? (
                                ...
                                <tr>
                                    <td colSpan="4" className="py-24 text-center">
                                        ...
                                        <h3 className="text-slate-900 font-bold text-xs uppercase tracking-tight">{t('admin_inbox.empty_title')}</h3>
                                        <p className="text-slate-400 text-[11px] font-bold mt-1">{t('admin_inbox.empty_subtitle')}</p>
                                    </td>
                                </tr>
                            ) : (
                                notifications.map((n) => (
