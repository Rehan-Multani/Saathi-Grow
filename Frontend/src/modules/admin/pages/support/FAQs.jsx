...
const FAQs = () => {
    ...
    const loadFAQs = async () => {
        ...
        try {
            ...
            showErrorAlert(t('common.error'), err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async (faqData) => {
        try {
            ...
            setShowModal(false);
            loadFAQs();
            const title = selectedFAQ ? t('support.faqs.update_success_title', { defaultValue: 'FAQ Updated!' }) : t('support.faqs.create_success_title', { defaultValue: 'FAQ Added!' });
            const message = selectedFAQ ? t('support.faqs.update_success_msg', { defaultValue: 'The FAQ has been successfully updated.' }) : t('support.faqs.create_success_msg', { defaultValue: 'New FAQ has been added successfully.' });
    ...
    const handleDelete = async (id) => {
        const result = await showDeleteConfirmation(
            t('support.faqs.delete_confirm_title', { defaultValue: 'Delete FAQ' }),
            t('support.faqs.delete_confirm_text', { defaultValue: 'Are you sure you want to delete this FAQ? It will be removed from the help center.' })
        );
        if (result.isConfirmed) {
            try {
                await deleteFAQ(token, id);
                loadFAQs();
                await showSuccessAlert(t('common.deleted'), t('support.faqs.delete_success_msg', { defaultValue: 'FAQ has been deleted.' }));
            } catch (err) {
                showErrorAlert(t('common.error'), err.message);
            }
        }
        ...
        <div className="p-3">
            <Card className="border-0 shadow-sm mb-4 bg-white rounded-3xl overflow-hidden">
                <Card.Body className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 p-4">
                    <div>
                        <h5 className="mb-0 fw-black text-dark tracking-tight">{t('support.faqs.title')}</h5>
                        <p className="text-muted small mb-0 mt-1 italic opacity-75">{t('support.faqs.subtitle')}</p>
                    </div>
                    <div className="d-flex flex-column flex-sm-row gap-2 flex-grow-1 justify-content-sm-end">
                        ...
                        <Button
                            ...
                            onClick={handleAdd}
                        >
                            <Plus size={16} strokeWidth={3} /> {t('support.faqs.add_new')}
                        </Button>
                    </div>
                </Card.Body>
            </Card>
                        ...
                        >
                            {filtered.length > 0 ? filtered.map((faq) => (
                                ...
                                <div className="py-12 text-center">
                                    ...
                                    <p className="text-gray-400 font-bold small uppercase tracking-widest">{t('support.faqs.no_results')}</p>
                                    <p className="text-gray-300 text-[11px] italic mt-1">{t('support.faqs.try_different')}</p>
                                </div>
                            )}
                        </Accordion>
                    )}
