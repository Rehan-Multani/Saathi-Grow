        ...
        <div className="container-fluid py-6 bg-slate-50/20 min-h-screen px-4 md:px-6 max-w-7xl mx-auto font-sans text-slate-800">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    >
                        <MessageSquare size={16} /> {t('payout_report.sidebar.contact_btn')}
                    </button>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                <div className="lg:col-span-2 space-y-8 mt-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {[
                            { label: t('details.stats.earnings'), value: `₹${(stats.totalEarnings || 0).toLocaleString()}`, icon: <TrendingUp size={18} />, color: 'emerald' },
                            { label: t('details.stats.orders'), value: stats.totalOrders || 0, icon: <ShoppingBag size={18} />, color: 'amber' },
                            { label: t('details.stats.products'), value: stats.totalProducts || 0, icon: <Package size={18} />, color: 'blue' },
                            { label: t('details.stats.pending'), value: `₹${(stats.pendingPayout || 0).toLocaleString()}`, icon: <IndianRupee size={18} />, color: 'rose' }
                        ].map((stat, i) => (
                    ...
                    <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-500">
                        <div className="p-6 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center shrink-0">
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-tight">{t('details.best_sellers.title')}</h3>
                            <button onClick={() => navigate(`/admin/vendors/products?vendorId=${vendor._id}`)} className="text-[10px] font-bold text-blue-600 uppercase tracking-widest hover:underline flex items-center gap-1.5 border-none bg-transparent">
                                {t('details.best_sellers.view_all')} <ExternalLink size={12} />
                            </button>
                        </div>
                        <div className="overflow-x-auto scrollbar-thin">
                            <table className="w-full text-left font-medium">
                                <thead className="bg-slate-50/50 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                    <tr>
                                        <th className="px-8 py-5">{t('details.best_sellers.table.name')}</th>
                                        <th className="px-6 py-5 text-center">{t('details.best_sellers.table.price')}</th>
                                        <th className="px-6 py-5 text-center">{t('details.best_sellers.table.sales')}</th>
                                        <th className="px-6 py-5 text-center">{t('details.best_sellers.table.stock')}</th>
                                        <th className="px-8 py-5 text-right">{t('details.best_sellers.table.status')}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                        ...
                                        <tr key={i} className="hover:bg-slate-50/20 transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden shrink-0 group-hover:scale-105 transition-transform shadow-inner">
                                                        {p.image ? <img src={p.image} className="w-full h-full object-cover" /> : <Package size={16} className="text-slate-200" />}
                                                    </div>
                                                    <div className="text-xs font-bold text-slate-800 uppercase tracking-tight group-hover:text-blue-600 transition-colors truncate max-w-[150px]">{p.name}</div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-6 text-center">
                                                <span className="text-[11px] font-bold text-slate-900 tracking-tighter italic">₹{(p.basePrice || p.price)?.toLocaleString()}</span>
                                            </td>
                                            <td className="px-6 py-6 text-center">
                                                <span className="text-xs font-bold text-blue-600 tracking-tight">{p.sales || 0} <span className="text-[9px] text-slate-400 uppercase tracking-tighter">Units</span></span>
                                            </td>
                                            <td className="px-6 py-6 text-center">
                                                <div className="flex flex-col items-center">
                                                    <span className={`text-xs font-bold tracking-tight ${p.stock < 20 ? 'text-rose-500' : 'text-slate-700'}`}>{p.stock || 0}</span>
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none opacity-60 mt-0.5 italic">{t('products.table.status')}</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6 text-right">
                                                <span className={`px-2.5 py-1 rounded-lg text-[9px] font-bold border uppercase tracking-widest ${p.stock > 0 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100'}`}>
                                                    {p.stock > 0 ? t('all_vendors.status.active') : t('payouts.status.rejected')}
                                                </span>
                                            </td>
                                        </tr>
                                    ))}
                                    {(!vendor.topProducts || vendor.topProducts.length === 0) && (
                                        <tr>
                                            <td colSpan="5" className="py-12 text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest italic leading-none">{t('all_vendors.no_data')}</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 overflow-hidden sticky top-6">
                        <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-50 pb-4">{t('details.profile.title')}</h3>
                        <div className="space-y-6">
                            {[
                        ...
                        <div className="mt-10 space-y-6 border-t border-slate-50 pt-8">
                            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{t('details.ops.title')}</h4>
                            <div className="space-y-5">
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">{t('details.ops.completion')}</span>
                                        <span className="text-emerald-600 text-xs font-bold tracking-tighter leading-none">{stats.successRate || 0}%</span>
                                    </div>
                                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                                        <div className="h-full bg-emerald-500 rounded-full transition-all duration-1000 shadow-sm" style={{ width: `${stats.successRate || 0}%` }} />
                                    </div>
                                </div>
                                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex gap-3">
                                    <ShieldCheck size={16} className="text-emerald-500 shrink-0" />
                                    <p className="text-[9px] text-slate-500 font-bold leading-relaxed uppercase tracking-tighter italic">{t('details.ops.verified_msg')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
