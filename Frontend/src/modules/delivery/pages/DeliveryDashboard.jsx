        ...
        <div className="max-w-md mx-auto space-y-4 pb-20">
            ...
            <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-sm font-black text-slate-900 leading-none">Hello, {profile?.name?.split(' ')[0]}</h2>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight mt-1">{profile?.vehicleType} • {profile?.uniqueId}</p>
                    </div>
                </div>
                <button 
                    onClick={() => refreshAll()}
            ...
            <div className="px-1">
                <div className="bg-slate-900 text-white p-3 px-4 rounded-2xl flex items-center justify-between shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[8px] font-black uppercase tracking-[0.2em] opacity-40 mb-0.5">Tactical Presence</span>
                        <div className="flex items-center gap-2">
                            <span className="text-sm font-black tracking-tight">{isOnline ? 'DUTY ONLINE' : 'DUTY OFFLINE'}</span>
                    ...
                    <div 
                        ...
                        className={`w-14 h-8 rounded-full cursor-pointer transition-colors duration-500 relative flex items-center px-1 ${
                ...
                {activeRun ? (
                    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden group">
                        ...
                        <div className="h-40 bg-slate-100 relative">
                            <img 
                                src={`https://maps.googleapis.com/maps/api/staticmap?center=22.7533,75.8948&zoom=14&size=400x200&maptype=roadmap&markers=color:black%7Clabel:S%7C22.7533,75.8948&markers=color:red%7Clabel:C%7C22.7196,75.8577&key=${GOOGLE_MAPS_API_KEY}`}
                                alt="Tactical Map" 
                            ...
                            <button 
                                ...
                                className="absolute bottom-3 right-3 bg-slate-900 text-white p-3 rounded-2xl shadow-xl active:scale-95 transition-all flex items-center gap-2"
                            >
                        ...
                        <div className="p-5 pt-2 space-y-4">
                            <div className="flex items-start gap-4">
                                <div className="flex flex-col items-center gap-1 mt-1">
                                    <div className="w-2.5 h-2.5 rounded-full bg-slate-900 border-2 border-white shadow-sm ring-2 ring-slate-100"></div>
                                    <div className="w-0.5 h-8 bg-slate-100 dashed"></div>
                                    <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-sm ring-2 ring-emerald-100"></div>
                                </div>
                                <div className="flex-1 space-y-4">
                                    <div>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight">Pickup Point</p>
                                        <h4 className="text-sm font-black text-slate-800">{activeRun.branchId?.name || 'Central Branch'}</h4>
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight">Delivery Target</p>
                                        <h4 className="text-sm font-black text-slate-800">{currentStop?.order?.user?.name || 'Client Location'}</h4>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="bg-slate-50 text-slate-500 px-2 py-0.5 rounded-lg text-[9px] font-black tracking-tight border border-slate-100">COD: {currentStop?.order?.totalAmount ? formatCurrency(currentStop.order.totalAmount) : 'N/A'}</span>
                                            <span className="bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-lg text-[9px] font-black tracking-tight border border-emerald-100">OTP SECURED</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button 
                                ...
                                className="w-full py-4 bg-slate-900 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-slate-200 flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
                            >
            ...
            <div className="grid grid-cols-2 gap-3 px-1">
                <button 
                    ...
                    className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-3 active:scale-95 transition-all shadow-sm group"
                >
