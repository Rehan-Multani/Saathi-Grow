...
import SummaryCards from '../../../store-manager/components/SummaryCards';
...
const StaffReports = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const res = await getStrategicAnalytics(staffUser.token);
            if (res.success) {
                setData(res);
            } else {
    ...
    const stats = [
        { label: 'Inventory Value', value: data ? `₹${data.summary.inventoryValue.toLocaleString()}` : '₹0', icon: 'DollarSign', color: 'bg-emerald-500', textColor: 'text-emerald-500', trend: 8.5, path: '/staff/inventory' },
        { label: 'Product Range', value: data ? data.summary.totalSku.toString() : '0', icon: 'Package', color: 'bg-blue-600', textColor: 'text-blue-600', trend: 12.1, path: '/staff/inventory' },
        { label: 'Bestseller', value: data ? data.summary.topProduct : 'N/A', icon: 'TrendingUp', color: 'bg-violet-600', textColor: 'text-violet-600', trend: 5.4, path: '/staff/inventory' },
        { label: 'Active Alerts', value: data ? data.summary.alerts.toString() : '0', icon: 'AlertTriangle', color: 'bg-amber-500', textColor: 'text-amber-500', trend: -2.3, path: '/staff/inventory' },
    ];
    ...
    const handleExport = () => {
        ...
        try {
            ...
            link.setAttribute("download", `staff_analytics_report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 text-left">
            {/* Header Section */}
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="space-y-6">
                        {(data?.assetDistribution || []).map((cat) => {
                            ...
                            return (
                                <div key={cat.name} className="space-y-2 group/item cursor-pointer" onClick={() => navigate('/staff/inventory')}>
