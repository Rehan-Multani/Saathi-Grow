...
const ProductCard = ({ product, isCompact = false }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-[#e8f5e9] to-[#ffffff] dark:from-[#141414] dark:to-[#141414] md:bg-none md:bg-white md:dark:bg-[#111111] rounded-2xl p-3 sm:p-5 shadow-[0_4px_12px_rgba(0,0,0,0.08)] md:shadow-[0_2px_8px_rgba(0,0,0,0.06)] border border-gray-200/60 dark:border-white/10 hover:border-[#0c831f]/30 active:border-[#0c831f]/30 dark:hover:border-white/40 hover:shadow-lg hover:shadow-green-500/10 active:shadow-md active:shadow-green-500/10 hover:scale-[1.01] active:scale-[0.98] transition-all duration-500 flex flex-col gap-2 sm:gap-3 h-auto md:h-full group relative overflow-hidden mb-1 ring-1 ring-[#0c831f]/5 md:ring-0">
      ...
      <div className="flex flex-col flex-grow px-1.5 sm:px-2">
        ...
        <div className="flex items-center justify-between mt-auto z-10">
          ...
          {quantity > 0 ? (
            <div className={`flex items-center bg-[#0c831f] text-white !rounded-full shadow-lg ${isCompact ? 'h-[18px] sm:h-[30px] min-w-[40px] sm:min-w-[70px]' : 'h-[22px] sm:h-[36px] min-w-[50px] sm:min-w-[85px]'} border border-[#0c831f]`}>
              <button
              ...
              >
                <Minus size={isCompact ? 8 : 10} sm:size={14} strokeWidth={3} />
              </button>
              <span className={`${isCompact ? 'text-[8px]' : 'text-[10px]'} sm:text-[14px] font-black w-4 sm:w-7 text-center select-none leading-none`}>
                {quantity}
              </span>
              <button
                onClick={(e) => handleUpdateQuantity(e, 1)}
              ...
              >
                <Plus size={isCompact ? 8 : 10} sm:size={14} strokeWidth={3} />
              </button>
            </div>
          ) : (
            <button
              onClick={handleAddToCart}
              className={`${isCompact ? 'px-2 sm:px-3 h-[18px] sm:h-[30px] text-[7px]' : 'px-3 sm:px-4 h-[22px] sm:h-[34px] text-[8px]'} py-1 bg-[#0c831f] text-white border border-transparent hover:bg-[#0a6b19] active:scale-95 transition-all sm:text-[11px] font-black !rounded-full uppercase tracking-wider shadow-sm flex items-center justify-center`}
            >
              ADD
            </button>
          )}
