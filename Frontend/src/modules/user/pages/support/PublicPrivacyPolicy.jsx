...
import { ChevronLeft, Shield, CheckCircle2, ArrowRight, Loader } from 'lucide-react';
...
const PublicPrivacyPolicy = () => {
    ...
    useEffect(() => {
        const fetchPolicy = async () => {
            setLoading(true);
            try {
                const data = await getPolicyContent('user-privacy-policy', 'User');
                setPolicy(data);
            } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-black transition-colors duration-300 pb-16">
            ...
            <div className="bg-gradient-to-br from-[#0c831f] via-[#096a19] to-black text-white py-16 px-6 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="text-left">
                        ...
                        <h1 className="text-3xl md:text-5xl font-black tracking-tight mb-3">Privacy & Trust Center</h1>
                        <p className="text-white/80 text-xs md:text-sm font-semibold max-w-lg leading-relaxed">
                            At SaathiGro, we value your trust. Learn how we handle your personal data to provide a seamless, secure experience.
                        </p>
                    </div>
                    <div className="hidden md:flex w-24 h-24 bg-white/10 rounded-3xl border border-white/20 items-center justify-center backdrop-blur-md animate-pulse">
                        <Shield size={48} className="text-[#CCFF00]" />
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4 mt-8">
                <div className="bg-white dark:bg-[#121212] rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm p-6 md:p-10">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20">
                            <Loader size={40} className="animate-spin text-[#0c831f] mb-4" />
                ...
                {/* FAQ Prompt */}
                <div className="mt-8 bg-gradient-to-r from-gray-900 to-black rounded-3xl p-6 md:p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="text-left">
                        <h4 className="text-lg font-black tracking-tight mb-1">Have questions about your data?</h4>
                        <p className="text-white/60 text-xs font-semibold">
                            Get details on how we store, handle, and secure your information.
                        </p>
                    </div>
                    <button
                        onClick={() => navigate('/support/user')}
                        className="bg-white text-black hover:bg-gray-100 py-3.5 px-6 rounded-xl font-black text-xs uppercase tracking-wider flex items-center gap-2 transition-all active:scale-95 w-full md:w-auto justify-center"
                    >
            ...
            <style>{`
                .legal-content h1 { font-size: 1.5rem; font-weight: 900; color: #0c831f; margin-top: 2rem; margin-bottom: 1rem; }
