        ...
        <div className="space-y-6 pb-20 md:pb-0">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Track and update your inventory levels</p>
                </div>
                <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                    <button
                        onClick={handleSync}
                        disabled={isSyncing}
                        className="flex-1 sm:flex-none px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors"
                    >
                    ...
                    {hasAnyChanges && (
                        <>
                            <button
                                ...
                                className="flex-1 sm:flex-none px-4 py-2 bg-red-50 text-red-600 border border-red-100 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-100 transition-colors"
                            >
                                <RotateCcw size={16} /> Reset
                            </button>
                            <button
                                onClick={handleSaveAll}
                                className="flex-1 sm:flex-none px-4 py-2 bg-[#0c831f] text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-[#0a6b19] shadow-sm transition-colors animate-in fade-in slide-in-from-right-2"
                            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs text-gray-500 font-bold uppercase">Stock Value</p>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 sticky top-16 z-20 md:static">
                ...
                <div className="flex gap-2 text-sm font-medium overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
                    <button
                        onClick={() => setFilterStatus('all')}
                        className={`px-4 py-2 rounded-lg whitespace-nowrap border transition-colors ${filterStatus === 'all' ? 'bg-[#0c831f] text-white border-[#0c831f]' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                    >
                        All Items
                    </button>
                    <button
                        onClick={() => setFilterStatus('low')}
                        className={`px-4 py-2 rounded-lg whitespace-nowrap border transition-colors ${filterStatus === 'low' ? 'bg-[#0c831f] text-white border-[#0c831f]' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                    >
                        Low Stock
                    </button>
                    <button
                        onClick={() => setFilterStatus('out')}
                        className={`px-4 py-2 rounded-lg whitespace-nowrap border transition-colors ${filterStatus === 'out' ? 'bg-[#0c831f] text-white border-[#0c831f]' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                    >
                        Out of Stock
                    </button>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[700px]">
                        <thead className="bg-gray-50 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Unit Value</th>
                                    ...
                                    <tr key={product.id} className={`transition-colors ${isUnsaved ? 'bg-green-50/50' : 'hover:bg-gray-50'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`w-12 text-center font-bold ${currentStock === 0 ? 'text-red-500' : isUnsaved ? 'text-[#0c831f]' : 'text-gray-900'}`}>
                                                    {currentStock}
                                                </span>
                                                <button onClick={() => handleStockChange(product.id, 1)} className="p-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200 shadow-sm border border-gray-200 active:scale-95 transition-all">
                                                    <Plus size={14} />
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${currentStock === 0 ? 'bg-red-50 text-red-600 border-red-100' :
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-xs font-bold rounded-lg flex items-center gap-1 ml-auto transition-all ${isUnsaved
            ...
            <div className="md:hidden space-y-4">
                {filteredProducts.map((product) => {
                    ...
                    const isUnsaved = hasUnsavedChanges(product.id);
                    return (
                        <div key={product.id} className={`bg-white p-4 rounded-xl shadow-sm border transition-colors ${isUnsaved ? 'border-[#0c831f] bg-green-50/10' : 'border-gray-100'}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">{product.category} • {formatCurrency(product.price)}</p>
                                        </div>
                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${currentStock === 0 ? 'bg-red-50 text-red-600 border-red-100' :
                                            currentStock < 20 ? 'bg-yellow-50 text-yellow-700 border-yellow-100' :
                                                'bg-green-50 text-green-700 border-green-100'
                                            }`}>
                                            {currentStock === 0 ? 'Out of Stock' : currentStock < 20 ? 'Low' : 'In Stock'}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-end justify-between mt-3">
                                        <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-1 border border-gray-100">
                                            ...
                                            <span className={`w-8 text-center font-bold text-sm ${currentStock === 0 ? 'text-red-500' : isUnsaved ? 'text-[#0c831f]' : 'text-gray-900'}`}>
                                                {currentStock}
                                            </span>
                                            <button onClick={() => handleStockChange(product.id, 1)} className="p-1 rounded bg-white text-gray-600 shadow-sm border border-gray-200 active:scale-95 transition-all">
                                                <Plus size={14} />
